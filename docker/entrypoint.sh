...
# Exported because envsubst substitutes only exported variables. Left
# unexported, the include path in nginx.conf.template renders empty and
# nginx refuses to start with `open() "/real-ip.conf" failed` — which is
# what happened in SAMADCON, and reaches a user as a service that will not
# come up rather than as anything a test could have read.
export SAMFSCON_CONF_DIR
...
#
# Behind a reverse proxy every request arrives from the proxy, so the address
# nginx sees is the proxy's — and the audit log would then record that same
# address for every administrator alike. X-Forwarded-For carries the real one,
# but it is a plain header and a client can send its own, so believing it
# unconditionally lets anyone write their own address into the log.
#
# The trusted hops are therefore named here and nowhere else. Empty by default,
# which means believe nobody and use the address that actually connected.
# --------------------------------------------------------------------------
REAL_IP_CONF="${SAMFSCON_CONF_DIR}/real-ip.conf"
{
    echo "# Generated by entrypoint.sh from SAMFSCON_TRUSTED_PROXIES."
    if [ -n "${SAMFSCON_TRUSTED_PROXIES:-}" ]; then
        # Commas or spaces, so the variable can be written either way.
        for proxy in $(printf '%s' "${SAMFSCON_TRUSTED_PROXIES}" | tr ',' ' '); do
            echo "set_real_ip_from ${proxy};"
        done
        echo "real_ip_header X-Forwarded-For;"
        # Walks the chain from the right and stops at the first address that is
        # not a trusted hop. That address is the caller.
        echo "real_ip_recursive on;"
    else
        echo "# SAMFSCON_TRUSTED_PROXIES is unset: no forwarded header is believed."
    fi
} > "${REAL_IP_CONF}"
...
if [ -f "${SAMFSCON_CONF_DIR}/nginx.conf.template" ]; then
    ...
    envsubst '${REDIRECT_TARGET} ${SAMFSCON_TLS_CERT} ${SAMFSCON_TLS_KEY} ${SAMFSCON_CONF_DIR}' \
